package com.sevensegment.jobis.security.config;

import jakarta.servlet.http.HttpServletResponse;
import com.sevensegment.jobis.security.filter.JWTFilter;
import com.sevensegment.jobis.security.filter.LoginFilter;
import com.sevensegment.jobis.security.handler.CustomLogoutHandler;
import com.sevensegment.jobis.security.jwt.JWTUtil;
import com.sevensegment.jobis.security.model.service.CustomUserDetailsService;
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, AuthenticationManager authenticationManager, CustomLogoutHandler customLogoutHandler) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        // payment 경로 설정
                        .requestMatchers(HttpMethod.GET, "/api/payments/**", "/payments/confirm").hasAnyRole("USER", "ADMIN") // 조회는 USER와 ADMIN 허용
                        .requestMatchers(HttpMethod.POST, "/api/payments/confirm, /api/payments/request").hasAnyRole("USER", "ADMIN") // 등록은 USER와 ADMIN 허용
                        .requestMatchers(HttpMethod.PUT, "/api/payments").hasAnyRole("USER", "ADMIN")// 수정은 USER와 ADMIN 허용
                        .requestMatchers(HttpMethod.DELETE, "/payments").hasAnyRole("USER", "ADMIN") // 삭제는 USER와 ADMIN 허용
                        ...
                        .requestMatchers(HttpMethod.PUT, "/notice/detail/{no}").hasAnyRole("ADMIN")
                        ...
                        // 채용공고
                        .requestMatchers(HttpMethod.GET,"/jobPosting/**").hasAnyRole("USER", "ADMIN") // 조회는 USER와 ADMIN 허용
